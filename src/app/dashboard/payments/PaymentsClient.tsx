...
export default function PaymentsClient({
  ...
  // Derived state — payer's queue is ReadyToPay (already authorized by accountant).
  const toPayInvoices = invoices.filter(inv => inv.status === 'ReadyToPay');
  ...
  const confirmPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setFileError(err instanceof Error ? err.message : 'Error processing payment. The invoice remains in ReadyToPay state — please try again.');
    } finally {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center">
          ...
          <div>
            ...
            <span className="text-xs text-gray-400">{toPayCount} invoice{toPayCount !== 1 ? 's' : ''} to transfer</span>
          </div>
        </div>
